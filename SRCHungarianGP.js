...
warn = function(ship_number, rule, time = 25, character = "Zoltar") {
  ...
  if (rule == 1) {
    ...
    echo("Wrong rule number (should be 1 to 5)");
  }
  var ship = game.ships[ship_number];
  ship.instructorSays(message, character);
  setTimeout(function() {
    ship.hideInstructor();
  }, time * 1500);
...
Flag = function(flag, message, time, character, ship, game) { 
  if (flag == "Y") {
  ...
  if (flag == "VSC") {
    message = "\n \n Virtual safety car!\n All ships is immobilized.(for 10-30 sec). I need to kick a troll/ there was an incident I need to fix"
    time = 30
    character = "Maria"
    for (var ship of game.ships){
