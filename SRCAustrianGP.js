...
"99999999999999999999      L                   L         B B B B B B  L       99999999999999999999999\n"+    //80
"999999999999999999999                                   L   L   L        LLLL  999999999999999999999\n"+ //31
"999999999999999999999999999999999999999999999999999999999999999999999999999     99999999999999999999\n"+ //32
...
checkpoints: [
  {x:StartX,y:StartY,direction:Math.PI,width:150}, // Start/ finish line
...
// Hack to prevent game start on fatal error

if (!window.PrivateServerFinder_old) {
  let modding = game.modding;
  window.PrivateServerFinder_old = window.PrivateServerFinder;
  window.PrivateServerFinder = function() {
    if (modding.context.stop) {
      modding.stopped();
    } else {
      return window.PrivateServerFinder_old.apply(this, arguments);
    }
  };
}
...
var map_size = this.options.map_size;
var map_lines = map.split('\n');
...
var safe_map = {};
var lap_map = {};
var lap_map_size = map_size * lap_map_precision;
for (var y = 0; y < lap_map_size; y++) {
  safe_map[y] = {};
  lap_map[y] = {};
}
var map_y = 0;
  ...
  for (var symbol of line) {
    if (!/\d/.test(symbol)) {
      ...
      for (var lap_y = lap_y_start; lap_y < lap_y_end; lap_y++) {
        var y = lap_y % lap_map_size;
        if (y < 0) {
          y += lap_map_size;
        }
        for (var lap_x = lap_x_start; lap_x < lap_x_end; lap_x++) {
          var x = lap_x % lap_map_size;
          if (x < 0) {
            x += lap_map_size;
          }
          lap_map[y][x] = true;
...
// offset to take care of ship position ranges
// map(20) x:  -100  ... 99.(9)
// map(20) y: 99.(9) ... -100

var pos_offset = 10 * map_size / 2;
var pos_div = 10 / lap_map_precision;

function lap_pos(ship) {
  var x = Math.trunc((pos_offset + ship.x) / pos_div) % lap_map_size;
  var y = Math.trunc((pos_offset - ship.y) / pos_div) % lap_map_size;
  if (x < 0) {
    x += lap_map_size;
  }
  if (y < 0) {
...
function is_on_safe(lap_pos) {
  return safe_map[lap_pos.y][lap_pos.x];
}

function is_on_lap(lap_pos) {
  return lap_map[lap_pos.y][lap_pos.x];
}

function bind_outlap(ship) {
  return function() {
    ship.custom.outlap_timer = 0;
    if (!is_on_lap(lap_pos(ship))) {
      ship.set({
        x: ship.custom.safe_x,
        y: ship.custom.safe_y,
        vx: 0,
        vy: 0,
        invulnerable: onlap_blink_time,
        healing: false,
      });
    }
  }
...
var checkShip = function(ship, index) {
  ...
  if (ship.alive) {
    var pos = lap_pos(ship);
    if (is_on_safe(pos)) {
      ship.custom.safe_x = ship.x;
      ship.custom.safe_y = ship.y;
      clear_outlap(ship);
    } else if (is_on_lap(pos)) {
      clear_outlap(ship);
    } else if (!ship.custom.outlap_timer && !ship.healing) {
      ship.custom.outlap_timer = setTimeout(bind_outlap(ship), outlap_delay);
